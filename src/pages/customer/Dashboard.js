...
export default function Dashboard(props) {
    ...
    // Setting up fleets state
    const [fleets, setFleets] = useState([])

    // Getting fleet data from local datasource
    let url = "http://localhost:8000/fleets"
    const getData = () => {
        var requestOptions = {
            method: "GET",
            redirect: "follow",
            ...
            <aside className="hidden sm:flex sm:flex-col">
                ...
                <div className="flex-grow flex flex-col justify-between text-gray-500 bg-gray-800">
                    <nav className="flex flex-col mx-4 my-6 space-y-4">
                        ...
                        <Link to="/fleet" className="inline-flex items-center justify-center py-3 hover:text-gray-400 hover:bg-gray-700 focus:text-gray-400 focus:bg-gray-700 rounded-lg">
                ...
                <main className="p-6 sm:p-10 space-y-6 bg-gray-800">
                    ...
                    <section className="grid md:grid-cols-2 xl:grid-cols-2 gap-6">
                        <div className="flex items-center p-8 bg-white shadow rounded-lg">
                            ...
                            <div>
                                <span className="block text-2xl font-bold">{props.fleetCount}</span>
                        ...
                        <div className="row-span-3 bg-white shadow rounded-lg">
                            <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                                <table class="w-full text-l text-center rtl:text-right text-black dark:text-black">
                                    ...
                                    <tbody>
                                        {fleets.map((fleet) =>
                                            <tr key={fleet.id} class="text-l text-black bg-white border-b dark:bg-white hover:bg-gray-50 dark:hover:bg-gray-100">
                                                <td className="px-6 py-4">{fleet.id}</td>
                                                <td className="px-6 py-4">{fleet.name}</td>
                                                <td className="px-6 py-4">{fleet.drones.length} Drones</td>
                                            </tr>)}
                                    </tbody>
                                </table>
                            </div>
