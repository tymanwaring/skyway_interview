...
export default function FleetManager(props) {
    ...
    // states to handle fleet update
    const [fleets, setFleets] = useState([])
    const [update, setUpdate] = useState(false)

    // Feta data from json server
    ...
    //get data on componant mount and when data is updated
    useEffect(() => {
        getData()
        setUpdate(false)
    }, [update]);

    // Rending Fleet Manager
    return (
            ...
            <div className="flex-grow text-gray-800">
                ...
                <main className="p-6 sm:p-10 space-y-6 bg-gray-800">
                    <div className="flex flex-col space-y-6 md:space-y-0 md:flex-row justify-between">
                        <div className="mr-6">
                            <h1 className="text-4xl font-semibold mb-2 text-white">Fleet Dashboard</h1>
                        </div>
                    </div>
                    <section className="grid md:grid-cols-2 xl:grid-cols-2 gap-6">
                        <div className="flex items-center p-8 bg-white shadow rounded-lg">
                            <Modal handler={setUpdate}></Modal>
                            <div>
                                <span className="block text-2xl font-bold">{fleets.length}</span>
                                <span className="block text-gray-500">Fleets</span>
                            </div>
